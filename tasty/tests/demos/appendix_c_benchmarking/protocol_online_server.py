...
def protocol(client, server, params):
    ...
    server.x = Unsigned(bitlen=32, empty=True, signed=False, dim=[1]).input(src=driver, desc='x')
    server.y = Unsigned(bitlen=32, empty=True, signed=False, dim=[1]).input(src=driver, desc='y')
    server.hx = Garbled(val=server.x, signed=False, bitlen=32, dim=[1])
    server.hy = Garbled(val=server.y, signed=False, bitlen=32, dim=[1])
