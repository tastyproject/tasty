...
def protocol(client, server, params):
    ...
    server.hx = Garbled(val=Unsigned(bitlen=32, dim=[1], signed=False, passive=True, empty=True), signed=False, bitlen=32, dim=[1])
    server.hy = Garbled(val=Unsigned(bitlen=32, dim=[1], signed=False, passive=True, empty=True), signed=False, bitlen=32, dim=[1])
    conversions.Garbled_Garbled_send(server.hx, client.hx, 32, [1], False)
    conversions.Garbled_Garbled_send(server.hy, client.hy, 32, [1], False)
    client.hr = client.hx * client.hy
    client.r = Unsigned(val=client.hr, passive=True, signed=False, bitlen=64, dim=[1])
