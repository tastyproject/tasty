...
def protocol(client, server, params):
    ...
    dima = params['dima']
    dimb = params['dimb']
    client.a = UnsignedVec(bitlen=la, dim=dima).input(src=driver, desc="a")
    client.a.output(dest=driver, desc="cc")

    server.b = UnsignedVec(bitlen=lb, dim=dimb).input(src=driver, desc="b")
    server.hb = HomomorphicVec(val=server.b)
    client.gb <<= GarbledVec(val=server.hb)
    client.sc = UnsignedVec(val=client.gb)
