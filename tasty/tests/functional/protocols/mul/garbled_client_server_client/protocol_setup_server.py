...
def protocol(client, server, params):
    ...
    server.gb = Garbled(val=Unsigned(bitlen=140, dim=[1], signed=False, passive=True, empty=True), signed=False, bitlen=140, dim=[1])
    conversions.Garbled_Garbled_send(server.gb, client.gb, 140, [1], False)
    client.gc = client.ga * client.gb
    client.c = Unsigned(val=client.gc, passive=True, signed=False, bitlen=280, dim=[1])
